...
public class PoissonProcessArrivalTimesTest {
	...
	public void testPoissonProcess() {
		...
		final double expectedDynamism = 1 / arrivalTimesGenerator.getOrdersPerAnnouncement();
	 ...
	 */
	static boolean isPoissonProcess(Frequency observations, double intensity, double confidence, long scenarioLength) {
		final double lengthFactor = scenarioLength / 60d;
		final PoissonDistribution pd = new PoissonDistribution(intensity * lengthFactor);
