...
public class GendreauContextBuilder {
  ...
  public GendreauContextBuilder(RoadModel rm, PDPModel pm, Vehicle dv) {
    roadModel = rm;
    pdpModel = pm;
    vehicle = dv;
  }
  ...
  public GendreauContext buildInRepetition(Parcel p, boolean isInCargo,
      boolean isAssignedToVehicle) {
    return createContext(genericContext, p, isInCargo, isAssignedToVehicle);
  }

  public GendreauContext buildFromScatch(long time, Parcel p, boolean isInCargo,
      boolean isAssignedToVehicle) {
  ...
  protected GendreauContext createContext(GendreauContext gc, Parcel p,
      boolean isInCargo, boolean isAssignedToVehicle) {

    final int numWaiters = 0;// isInCargo ?
    // coordinationModel.getNumWaitersFor(p) : 0
    return new GendreauContext(gc.vehicleDTO, gc.truckPosition,
        gc.truckContents, p.getDto(), gc.time,
        isInCargo, isAssignedToVehicle, numWaiters, gc.otherVehiclePositions,
        new HashSet<Parcel>());
  }
  ...
  protected GendreauContext createGenericContext(long time) {
    ...
    return new GendreauContext(vehicle.getDTO(), roadModel.getPosition(vehicle),
        convert(contents), null, time,
