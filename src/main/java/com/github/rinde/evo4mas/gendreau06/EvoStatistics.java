...
public class EvoStatistics extends GPStats {
  ...
  public void setup(final EvolutionState state, final Parameter base) {
    ...
    final String dirName = new SimpleDateFormat("yyyyMMdd-HHmmss")
        .format(new Date());
    path = "runs/" + dirName;
  ...
  public static void createSymbolicLink(String target, String link) {
    final String[] cmd = { "ln", "-sFh", new File(target).getAbsolutePath(),
        new File(link).getAbsolutePath() };
    int exitCode;
    try {
      exitCode = Runtime.getRuntime().exec(cmd).waitFor();
      checkState(exitCode == 0, "ln signaled an error with exit code %s",
          exitCode);
  ...
  public void printMore(EvolutionState state, Individual best,
    ...
    final List<ResultDTO> testResults = newArrayList(
        ((Gendreau06Evaluator) state.evaluator)
  ...
  public void finalStatistics(final EvolutionState state, final int result) {
    ...
    for (int y = 1; y < state.population.subpops[0].individuals.length; y++) {
      if (best == null || state.population.subpops[0].individuals[y].fitness
