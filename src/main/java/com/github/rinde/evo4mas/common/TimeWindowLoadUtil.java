...
public final class TimeWindowLoadUtil {
  ...
  static TimeWindow getOverlapInterval(TimeWindow tw1, TimeWindow tw2) {
    ...
    if (right - left > 0) {
      return TimeWindow.create(left, right);
    }
    return null;
  }

  public static double getOverlapLoad(TimeWindowLoad twl,
      List<TimeWindowLoad> list) {
    if (list.isEmpty()) {
      return twl.load;
    }
  ...
  public static double getMaxOverlapLoad(final TimeWindowLoad twl,
      List<TimeWindowLoad> list) {
    if (list.isEmpty()) {
      return twl.load;
    }
  ...
  public static double getMinOverlapLoad(TimeWindowLoad twl,
      List<TimeWindowLoad> list) {
    if (list.isEmpty()) {
      return twl.load;
    }
