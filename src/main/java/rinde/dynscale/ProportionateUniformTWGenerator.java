...
public class ProportionateUniformTWGenerator implements TimeWindowGenerator {
	 ...
	 * @param vehicleSpeed
	 */
	public ProportionateUniformTWGenerator(Point depotLocation, long endTime, long serviceTime, long minResponseTime,
			double vehicleSpeed) {
	...
	public ImmutableList<TimeWindow> generate(long orderAnnounceTime, Point pickup, Point delivery, RandomGenerator rng) {
		checkArgument(orderAnnounceTime <= endTime
				- (minResponseTime + travelTime(pickup, delivery) + (2 * serviceTime) + travelTime(delivery, depotLocation)), "The orderAnnounceTime is infeasible.");
		...
		final long latestDeliveryEndTime = endTime - (travelTime(delivery, depotLocation) + serviceTime);
		final long latestPickupEndTime = latestDeliveryEndTime - (travelTime(pickup, delivery) + serviceTime);

		/*
		 * PICKUP
		 */
		final long middlePickup = earliestPickupStartTime
				+ DoubleMath.roundToLong((latestPickupEndTime - earliestPickupStartTime) / 2, RoundingMode.HALF_EVEN);
		...
		if (rng.nextDouble() <= betaPickup) {
			pickLeft = earliestPickupStartTime
					+ DoubleMath
							.roundToLong((middlePickup - earliestPickupStartTime) * rng.nextDouble(), RoundingMode.HALF_EVEN);
		} else {
			pickLeft = middlePickup
					+ DoubleMath
							.roundToLong((latestPickupEndTime - middlePickup) * rng.nextDouble(), RoundingMode.HALF_EVEN);
		}
		final long pickupRemainingTime = latestPickupEndTime - pickLeft;
		final double deltaPickup = uniform(.1, .8, rng);
		long pickRight = pickLeft + DoubleMath.roundToLong(deltaPickup * pickupRemainingTime, RoundingMode.HALF_EVEN);

		/*
		 * DELIVERY
		 */
		final long earliestDeliveryStartTime = pickLeft + serviceTime + travelTime(pickup, delivery);
		...
		if (rng.nextDouble() <= betaDeliver) {
			deliverLeft = earliestDeliveryStartTime
					+ DoubleMath
							.roundToLong((middleDelivery - earliestDeliveryStartTime) * rng.nextDouble(), RoundingMode.HALF_EVEN);
		} else {
			deliverLeft = middleDelivery
					+ DoubleMath
							.roundToLong((latestDeliveryEndTime - middleDelivery) * rng.nextDouble(), RoundingMode.HALF_EVEN);
		}
		...
		final long deliverRight = deliverLeft
				+ DoubleMath.roundToLong(deltaDeliver * deliverRemainingTime, RoundingMode.HALF_EVEN);
		...
		return ImmutableList.of(new TimeWindow(pickLeft, pickRight), new TimeWindow(deliverLeft, deliverRight));
	}
	...
	long travelTime(Point p1, Point p2) {
		return DoubleMath.roundToLong(Point.distance(p1, p2) / vehicleSpeed, RoundingMode.CEILING);
	}
