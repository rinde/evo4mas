...
public class SolverBidder extends AbstractBidder {
    ...
    private double computeBid(DefaultParcel p, long time) {
        final Set<DefaultParcel> availableParcels =
                newLinkedHashSet(assignedParcels);
        if (p != null) {
        ...
        final StateContext sc =
                Solvers.convert(roadModel.get(), pdpModel.get(), vehicle.get(),
                    availableParcels, time, SI.MILLI(SI.SECOND),
                    NonSI.KILOMETERS_PER_HOUR, SI.KILOMETER);
        final ArraysObject ao =
                ArraysSolvers.toSingleVehicleArrays(sc.state,
                    SI.MILLI(SI.SECOND));
        final SolutionObject so =
