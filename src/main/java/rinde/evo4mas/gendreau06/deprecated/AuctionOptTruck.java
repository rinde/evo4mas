...
public class AuctionOptTruck extends
    ...
    void updateRoute(long time) {
        ...
        throw new RuntimeException("this code is deprecated!");
        // final SolutionObject sol = solver
        // .solve(travelTime, releaseDates, dueDates, servicePairs,
        // serviceTime);
        //
    ...
    public SolutionObject getSolutionObject() {
        ...
        return new SolutionObject(solutionObject.route,
                solutionObject.arrivalTimes, solutionObject.objectiveValue);
    }
    ...
    public static class Wait extends
            AbstractTruckState<OptEvent, AuctionOptTruck> {
        public OptEvent handle(OptEvent event, AuctionOptTruck context) {
            ...
            if (!context.route.isEmpty()
                    && !context
                            .isTooEarly(context.route.peek(), context.roadModel
                                    .getPosition(context), context.currentTime)) {
                return OptEvent.DONE;
            }

            if (context.route.isEmpty()
                    && context.isEndOfDay(context.currentTime)
                    && !context.roadModel.getPosition(context)
                            .equals(context.dto.startPosition)) {
                context.roadModel
                        .moveTo(context, context.dto.startPosition, context.currentTime);
            }
    ...
    public static class Goto extends
            AbstractTruckState<OptEvent, AuctionOptTruck> {
        public OptEvent handle(OptEvent event, AuctionOptTruck context) {
            if (context.pdpModel.getParcelState(context.route.peek()) == ParcelState.IN_CARGO) {
                // move to deliver location
                context.roadModel.moveTo(context, context.route.peek()
                        .getDestination(), context.currentTime);
                if (context.roadModel.getPosition(context).equals(context.route
                        .peek().getDestination())) {
                    return OptEvent.DONE;
                }
            } else {
                ...
                context.roadModel
                        .moveTo(context, context.route.peek(), context.currentTime);
                if (context.roadModel.equalPosition(context, context.route
                        .peek())) {
                    return OptEvent.DONE;
                }
            }
    ...
    public static class Service extends
        ...
        public void onEntry(OptEvent event, AuctionOptTruck context) {
            if (context.pdpModel.getParcelState(context.route.peek()) == ParcelState.IN_CARGO) {
                // deliver
                context.pdpModel
                        .deliver(context, context.route.peek(), context.currentTime);
            } else {
                context.pdpModel
                        .pickup(context, context.route.peek(), context.currentTime);
            }
