...
public class HeuristicSolver implements Solver {
    ...
    private SolutionObject performSteepestDescent(int[][] travelTime,
            ...
            int[] serviceTime, int[] pickupToDeliveryMap,
        ...
        while (improved) {
            ...
            for (int i = 1; i < n - 2; i++) {
                ...
                for (int j = i + 1; j < Math.min(deliveryLocation, n - 1); j++) {
                    final List<Integer> newPerm = new ArrayList<Integer>(
                            current);
                    ...
                    if (newSol.objectiveValue < bestSol.objectiveValue) {
                        ...
                        if (DEBUG) {
                            System.out
                                    .println("Found new best solution with objective: "
        ...
        for (int it = 0; it < maxIt; it++) {
            ...
            if (rand.nextBoolean()) {
                ...
                do {
                    ...
                    j = i
                            + 1
                            + rand.nextInt(Math.min(deliveryLocation, n - 1)
                                    - (i + 1));
                } while (!ok);
    ...
    private SolutionObject construct(int[] permutation, int[][] travelTime,
        ...
        for (int i = 1; i < n; i++) {
            ...
            arrivalTimes[next] = Math.max(previousT
                    + travelTime[previous][next], releaseDates[next]);
